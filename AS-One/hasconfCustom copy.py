...
def video_detection(path_x='', conf_=0.5, filter_classes=None):
    ...
    for j in range(nframes):
        ...
        # Check if there are any detections using len()
        if len(ouput_detected) > 0:
            print(f"First detection: {ouput_detected[0]}")
            for det in ouput_detected:
                total_detections += 1
                det = np.array(det).flatten()  # ensure it's a flat 1D array
                box = det[:4].astype(float).tolist()  # x1, y1, x2, y2 as floats
                conf = float(det[4])
                cls_id = int(det[5])
                ...
                det = np.array(det).flatten()  # Ensure it's 1D
